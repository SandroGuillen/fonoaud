...
export class FonoaudiologoController {
  constructor(private fonoaudiologoRepository: FonoaudiologoRepository) {}
  ...
  async getFonoaudiologo(req: Request, res: Response) {
    try {
      ...
      if (fonoaudiologo_id && typeof fonoaudiologo_id == "string") {
        const fonoaudiologoObjectId = ObjectId.createFromHexString(fonoaudiologo_id);
    ...
    try {
      const { fonoauiologo_id } = req.params;
            if (fonoauiologo_id && typeof fonoauiologo_id == "string") {
              const fonoauiologoObjectId = ObjectId.createFromHexString(fonoauiologo_id);
              await this.fonoaudiologoRepository.deleteOne({ _id: fonoauiologoObjectId });
